...
public class InMemoryStore : IStore {
    ...
    public Task CreateParticipantAsync(string sessionId, string participantId, string name) =>
        UpdateSession(sessionId, session => session with { Participants = [.. session.Participants, new(participantId, name, "", 0) ] });

    public Task AddPointAsync(string sessionId, string point) =>
        UpdateSession(sessionId, session => session with { Points = [.. session.Points, point] });
    ...
    public Task DeleteParticipantAsync(string sessionId, string participantId) =>
        UpdateSession(sessionId, session => session with { Participants = session.Participants.Where(p => p.ParticipantId != participantId).ToArray() });
    ...
    public Task IncrementParticipantStarsAsync(string sessionId, string participantId, int count = 1) =>
        UpdateParticipant(sessionId, participantId, participant => participant with { Stars = participant.Stars + count });

    public Task RemovePointAsync(string sessionId, string point) =>
        UpdateSession(sessionId, session => session with { Points = [.. session.Points.Except([point])] });

    public Task UpdateAllParticipantPointsAsync(string sessionId, string points = "") =>
        UpdateSession(sessionId, session => session with { Participants = [.. session.Participants.Select(p => p with { Points = points})] });

    public Task UpdateParticipantNameAsync(string sessionId, string participantId, string name) =>
        UpdateParticipant(sessionId, participantId, participant => participant with { Name = name });

    public Task UpdateParticipantPointsAsync(string sessionId, string participantId, string points) =>
        UpdateParticipant(sessionId, participantId, participant => participant with { Points = points });

    public Task UpdateSessionStateAsync(string sessionId, State state) =>
        UpdateSession(sessionId, session => session with { State = state });

    public Task UpdateSessionTitleAsync(string sessionId, string title) =>
        UpdateSession(sessionId, session => session with { Title = title });
}
