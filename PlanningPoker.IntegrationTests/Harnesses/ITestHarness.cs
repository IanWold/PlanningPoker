...
/// <summary>
/// Produces <see cref="Client.Client"/>/<see cref="SessionStore"/> pairs for a behavior test without the test knowing
...
public interface ITestHarness : IAsyncDisposable {
    Task<(Client.Client Client, SessionStore Store)> CreateClientAsync();
}
